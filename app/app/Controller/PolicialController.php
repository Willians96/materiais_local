...
class PolicialController
{
    ...
    public function pesquisarPM()
    {
        ...
        try {
            ...
            //verficar se o policial pertence a outo grande comando
            $cod_gcmdo_busca = substr($policial->codopm, 0, 3);
            $cod_gcmdo_usuario = substr($_SESSION['codopm'], 0, 3);
            //verifica os 3 primeiros dígitos do código

            if ($cod_gcmdo_busca != $cod_gcmdo_usuario && $_SESSION['cod_perfil'] != '1' ) {
                $_SESSION['msgText'] = 'Policial pertence a outro Grande Comando.<br> '.$policial->ptgr.' '. $policial->nome . ' - '. $policial->opm;    
                $_SESSION['msg'] = 'alert';
                header('location: ' . $url); exit;
            }
